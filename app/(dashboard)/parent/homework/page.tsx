...
export default async function ParentHomeworkPage() {
    ...
    return (
      <div className="flex flex-col flex-1">
        ...
        <main className="flex-1 p-6 flex items-center justify-center">
          <div className="text-center"><Users className="h-10 w-10 mx-auto text-white/30 mb-3" /><p className="text-white/40">No children linked.</p></div>
        </main>
      </div>
    );
  }
  ...
  const variantStyle = {
    submitted: { badge: "bg-emerald-500/10 text-emerald-400", dot: "bg-emerald-500" },
    pending:   { badge: "bg-amber-500/10 text-amber-400",   dot: "bg-amber-500"   },
    overdue:   { badge: "bg-rose-500/10 text-rose-400",     dot: "bg-rose-500"    },
  };
            ...
            <div key={childIds[idx]}>
              ...
              {homework.length === 0 ? (
                <div className="text-center py-8 bg-[#111318] rounded-2xl border border-white/[0.04]">
                  <BookOpen className="h-8 w-8 mx-auto text-white/30 mb-2" />
                  <p className="text-sm text-white/30">No homework found.</p>
                </div>
              ) : (
                <div className="bg-[#111318] rounded-2xl border border-white/[0.04] overflow-hidden divide-y">
                  {homework.map((h: any) => {
                      ...
                      <div key={h.id} className="px-5 py-4">
                        <div className="flex items-start justify-between gap-4">
                          <div className="min-w-0">
                            <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{h.subject?.name}</span>
                            <p className="text-sm font-bold text-white/80 mt-0.5">{h.homeworkTitle ?? h.description?.slice(0, 60)}</p>
