...
export function NotificationsClient({ settings: initial }: { settings: NotifRow[] }) {
  ...
  const Toggle = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
    <button
      type="button"
      onClick={onChange}
      className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${checked ? "bg-blue-600" : "bg-white/[0.06]"}`}
    >
      <span className={`inline-block h-3.5 w-3.5 rounded-full bg-[#111318] transition-transform shadow-sm ${checked ? "translate-x-4" : "translate-x-1"}`} />
    </button>
  );
  ...
  return (
    <main className="flex-1 p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-white/40 mt-0.5">Control which events trigger Email, SMS, and in-app push notifications.</p>
      </div>

      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm flex items-center gap-2">
            <Bell className="h-4 w-4 text-blue-400" /> Event Notifications
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <table className="w-full text-sm">
            <thead className="bg-[#0f1015] border-b">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-white/50">Event</th>
                <th className="text-center px-4 py-3 font-medium text-white/50">
            ...
            <tbody className="divide-y">
              {settings.map((s) => (
                <tr key={s.type} className="hover:bg-[#0f1015]">
