      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <Mail className="h-4 w-4 text-blue-400" /> SMTP Configuration
          </CardTitle>
          <p className="text-sm text-white/40">Configure the outgoing email server for notifications and alerts.</p>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPass(v => !v)} className="absolute right-2 top-2.5 text-white/30">
          ...
          <div className="md:col-span-2 flex items-center gap-2">
            <input type="checkbox" id="isActive" checked={form.isActive} onChange={(e) => set("isActive", e.target.checked)} className="h-4 w-4 rounded border-white/[0.08]" />
            <label htmlFor="isActive" className="text-sm text-white/60">Enable email sending</label>
          </div>
