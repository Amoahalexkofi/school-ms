  ...
  return (
    <main className="flex-1 p-6 space-y-5 bg-[#0f1015]">
      <div className="flex items-center gap-3">
        <Link href="/fees/setup" className="inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white/70 transition-colors">
          <ArrowLeft className="h-4 w-4" /> Back to Setup
        </Link>
      </div>

      {/* Header card */}
      <div className="bg-[#111318] rounded-xl border border-white/[0.06] shadow-sm p-5 flex items-start justify-between">
        <div>
          <h2 className="text-base font-semibold text-white/80">{sg.feeGroup.name}</h2>
          <p className="text-sm text-white/40 mt-0.5">Session: {sg.session.session}</p>
          <p className="text-xs text-white/30 mt-1">{sg._count.studentFeesMasters} student(s) assigned · {sg.items.length} fee item(s)</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-white/30">Total Amount</p>
          <p className="text-2xl font-bold text-white/80">₵{totalAmount.toLocaleString()}</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3 flex flex-row items-center justify-between">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            <DollarSign className="h-4 w-4 text-emerald-400" /> Fee Items
          </CardTitle>
        ...
        <CardContent className="p-0">
          {sg.items.length === 0 ? (
            <p className="text-sm text-white/30 text-center py-8">No items yet. Add fee types with amounts.</p>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {sg.items.map((item: Item) => (
                  <tr key={item.id} className="hover:bg-[#0f1015]">
                    <td className="px-4 py-3 font-medium text-white/80">{item.feeType.name}</td>
                    <td className="px-4 py-3 font-mono text-xs text-white/40">{item.feeType.code}</td>
                    <td className="px-4 py-3 font-semibold text-emerald-400">₵{Number(item.amount).toLocaleString()}</td>
                    <td className="px-4 py-3 text-white/40 text-xs">{item.dueDate ? new Date(item.dueDate).toLocaleDateString() : "—"}</td>
                    ...
                    <td className="px-4 py-3 flex gap-2 justify-end">
                      ...
                      <Button size="sm" variant="outline" className="text-red-400 border-red-500/20 hover:bg-red-500/10" onClick={() => handleDelete(item.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </td>
                  </tr>
                ))}
                <tr className="bg-[#0f1015] border-t">
                  <td colSpan={2} className="px-4 py-2.5 font-semibold text-white/60">Total</td>
                  <td className="px-4 py-2.5 font-bold text-emerald-400">₵{totalAmount.toLocaleString()}</td>
                  <td colSpan={3} />
                </tr>
              </tbody>
            </table>
      ...
      <div className="text-sm text-white/40">
        Ready to assign?{" "}
        <Link href={`/fees/assign?sgId=${sg.id}`} className="text-blue-400 hover:underline font-medium">
      ...
      <Dialog open={open} onOpenChange={o => !o && setOpen(false)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            {!editItem && (
              <div>
                <label className="block text-sm font-medium text-white/60 mb-1">Fee Type *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-white/60 mb-1">Fine Type</label>
              <select className="w-full h-9 rounded-lg border border-white/[0.08] px-3 text-sm bg-[#111318] focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            {form.fineType === "PERCENTAGE" && (
              <div>
                <label className="block text-sm font-medium text-white/60 mb-1">Fine %</label>
                <Input type="number" min="0" step="0.01" value={form.finePercentage} onChange={set("finePercentage")} />
              </div>
            )}
            {form.fineType === "AMOUNT" && (
              <div>
                <label className="block text-sm font-medium text-white/60 mb-1">Fine Amount (₵)</label>
