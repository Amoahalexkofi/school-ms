      ...
      <main className="flex-1 p-4 md:p-6 space-y-6 max-w-4xl mx-auto w-full">
        ...
        <div className="bg-gradient-to-r from-violet-600 to-purple-600 rounded-2xl p-6 text-white">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-[#111318]/15 rounded-xl p-4 text-center">
        ...
        {feeMasters.length === 0 ? (
          <div className="text-center py-16 bg-[#111318] rounded-2xl border border-white/[0.04]">
            <CreditCard className="h-10 w-10 mx-auto text-white/30 mb-3" />
            <p className="text-white/40 font-medium">No fee invoices found</p>
          </div>
        ) : (
                ...
                <div key={fm.id} className="bg-[#111318] rounded-2xl border border-white/[0.04] overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-4 gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${fullyPaid ? "bg-emerald-500/10" : "bg-rose-500/10"}`}>
                        {fullyPaid
                          ? <CheckCircle2 className="h-5 w-5 text-emerald-400" />
                          : <AlertCircle className="h-5 w-5 text-rose-500" />}
                      </div>
                      <div className="min-w-0">
                        <p className="font-bold text-white/80 text-sm truncate">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-lg font-black text-white/80">₵{(fm.amount ?? 0).toLocaleString()}</p>
                      {fullyPaid
                        ? <p className="text-xs text-emerald-400 font-bold">Fully Paid</p>
                        : <p className="text-xs text-rose-400 font-bold">Due: ₵{due.toLocaleString()}</p>}
                    </div>
                  </div>
