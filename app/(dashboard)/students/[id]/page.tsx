...
const ATT_STYLE: Record<string, string> = {
  ...
  A: "bg-red-500/10 text-red-400",
  L: "bg-amber-500/10 text-amber-400",
  H: "bg-violet-500/10 text-violet-400",
  F: "bg-orange-500/10 text-orange-400",
};
    ...
    <div className="flex flex-col flex-1">
      ...
      <main className="flex-1 p-6 space-y-6 max-w-5xl">
        <div className="flex items-center justify-between">
          <Link href="/students" className="inline-flex items-center gap-1.5 text-sm text-white/40 hover:text-white/70">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-white/40 font-mono">{student.admissionNo}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${student.isActive ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}>
                ...
                <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-2 text-sm">
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">Class</p>
                    <p className="text-white/60">{classLabel}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">Roll No</p>
                    <p className="text-white/60">{currentSession?.rollNo ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">Date of Birth</p>
                    <p className="text-white/60">{dob}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">Gender</p>
                    <p className="text-white/60">{student.gender ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">Blood Group</p>
                    <p className="text-white/60">{student.bloodGroup ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">Mobile</p>
                    <p className="text-white/60">{student.mobileNo ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">School House</p>
                    <p className="text-white/60">{student.schoolHouse?.name ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30 uppercase tracking-wide">Session</p>
                    <p className="text-white/60">{currentSession?.session.session ?? "—"}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold flex items-center gap-2">
                <User className="h-4 w-4 text-white/50" /> Guardian Information
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-2 text-sm">
                {student.fatherName && <>
                  <div>
                    <p className="text-xs text-white/30">Father&apos;s Name</p>
                    <p>{student.fatherName}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30">Father&apos;s Phone</p>
                    <p>{student.fatherPhone ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30">Father&apos;s Occupation</p>
                    <p>{student.fatherOccupation ?? "—"}</p>
                  </div>
                </>}
                {student.motherName && <>
                  <div>
                    <p className="text-xs text-white/30">Mother&apos;s Name</p>
                    <p>{student.motherName}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30">Mother&apos;s Phone</p>
                    <p>{student.motherPhone ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30">Mother&apos;s Occupation</p>
                    <p>{student.motherOccupation ?? "—"}</p>
                  </div>
                </>}
                {student.guardianName && <>
                  <div>
                    <p className="text-xs text-white/30">Guardian Name</p>
                    <p>{student.guardianName}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30">Guardian Phone</p>
                    <p>{student.guardianPhone ?? "—"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-white/30">Guardian Email</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold flex items-center gap-2">
                <ClipboardList className="h-4 w-4 text-blue-400" /> Attendance
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="text-center py-2">
                <p className={`text-4xl font-bold ${pct < 75 ? "text-red-400" : "text-emerald-400"}`}>{pct}%</p>
                <p className="text-xs text-white/30 mt-1">of {schoolDays.length} school days</p>
              </div>
              <div className="grid grid-cols-2 gap-2 text-center text-sm">
                <div className="bg-emerald-500/10 rounded-lg py-2">
                  <p className="font-bold text-emerald-400">{present}</p>
                  <p className="text-xs text-emerald-400">Present</p>
                </div>
                <div className="bg-red-500/10 rounded-lg py-2">
                  <p className="font-bold text-red-400">{absent}</p>
                  <p className="text-xs text-red-400">Absent</p>
                </div>
              </div>
              {pct < 75 && schoolDays.length > 0 && (
                <p className="text-xs text-red-400 bg-red-500/10 rounded p-2">Below 75% threshold</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold flex items-center gap-2">
                <DollarSign className="h-4 w-4 text-yellow-400" /> Fees
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-white/40">Total Invoiced</span>
                  <span className="font-medium">₵{totalInvoiced.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/40">Paid</span>
                  <span className="font-medium text-emerald-400">₵{totalPaid.toLocaleString()}</span>
                </div>
                <div className="flex justify-between border-t pt-2">
                  <span className="text-white/40">Balance</span>
                  <span className={`font-bold ${totalInvoiced - totalPaid > 0 ? "text-red-400" : "text-emerald-400"}`}>
                    ₵{(totalInvoiced - totalPaid).toLocaleString()}
                  </span>
                </div>
              </div>
              {student.feesMasters.slice(0, 3).map((fm: any) => (
                <div key={fm.id} className="text-xs text-white/50 truncate">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold flex items-center gap-2">
                <Calendar className="h-4 w-4 text-violet-400" /> Recent Days
              </CardTitle>
            </CardHeader>
            <CardContent>
              {student.attendances.length === 0 ? (
                <p className="text-xs text-white/30">No attendance records yet.</p>
              ) : (
                  ...
                  {student.attendances.slice(0, 30).map((a: any) => {
                    ...
                    return (
                      <div
                        ...
                        className={`w-6 h-6 rounded text-xs flex items-center justify-center font-bold cursor-default ${ATT_STYLE[kv] ?? "bg-white/[0.04] text-white/40"}`}
                      >
                        {kv}
                      </div>
                    );
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              <BookOpen className="h-4 w-4 text-indigo-400" /> Exam Results
            </CardTitle>
          </CardHeader>
          <CardContent>
            {publishedMarks.length === 0 ? (
              <p className="text-sm text-white/30">No published results yet.</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-[#0f1015]">
                  <tr>
                    <th className="text-left px-3 py-2 font-medium text-white/50">Exam</th>
                ...
                <tbody className="divide-y">
                  {publishedMarks.map((m: any) => (
                    <tr key={m.id} className="hover:bg-[#0f1015]">
                      <td className="px-3 py-2 text-white/40 text-xs">{m.examSchedule?.examGroup?.name}</td>
        ...
        {student.sessions.length > 1 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-semibold flex items-center gap-2">
                <GraduationCap className="h-4 w-4 text-teal-400" /> Enrollment History
              </CardTitle>
            </CardHeader>
            <CardContent>
              <table className="w-full text-sm">
                <thead className="bg-[#0f1015]">
                  <tr>
                    <th className="text-left px-3 py-2 font-medium text-white/50">Session</th>
                    <th className="text-left px-3 py-2 font-medium text-white/50">Class / Section</th>
                    <th className="text-left px-3 py-2 font-medium text-white/50">Roll No</th>
                    <th className="text-left px-3 py-2 font-medium text-white/50">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {student.sessions.map((ss: any) => (
                    <tr key={ss.id} className="hover:bg-[#0f1015]">
                      <td className="px-3 py-2 text-white/50">{ss.session.session}</td>
                      <td className="px-3 py-2">{ss.classSection.class.name} – {ss.classSection.section.name}</td>
                      <td className="px-3 py-2 text-white/40">{ss.rollNo ?? "—"}</td>
                      <td className="px-3 py-2">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${ss.isAlumni ? "bg-white/[0.04] text-white/50" : ss.isActive ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}>
        ...
        {student.disableReason && (
          <Card className="border-red-500/20 bg-red-500/10">
            <CardContent className="pt-4">
              <p className="text-sm font-medium text-red-400">Disable Reason</p>
              <p className="text-sm text-red-400 mt-1">{student.disableReason}</p>
