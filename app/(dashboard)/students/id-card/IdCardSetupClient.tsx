  ...
  return (
    <main className="flex-1 p-6 space-y-5 max-w-4xl mx-auto">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-base font-semibold text-white/80">ID Card Templates</h2>
          <p className="text-xs text-white/30 mt-0.5">Configure what fields appear on printed student ID cards.</p>
        </div>
      ...
      {templates.length === 0 && (
        <Card><CardContent className="py-10 text-center text-sm text-white/30">No templates yet. Create one to enable ID card printing.</CardContent></Card>
      )}
            ...
            <CardContent className="pt-4 space-y-3">
              ...
              <div className="rounded-lg overflow-hidden text-xs" style={{ backgroundColor: t.bgColor, color: t.fontColor }}>
                ...
                <div style={{ backgroundColor: t.bodyColor, color: "#111" }} className="px-3 py-2">
                  ...
                  <p className="text-white/30 mt-0.5">{(t.fieldList ?? []).slice(0, 3).join(" · ")||"(no fields)"}</p>
                </div>
              </div>
              <div className="flex items-center justify-between">
                {t.status === 1
                  ? <span className="text-xs font-medium text-blue-400 flex items-center gap-1"><Star className="h-3 w-3" /> Default</span>
                  : <button onClick={() => setActive(t.id)} className="text-xs text-white/30 hover:text-blue-400">Set as default</button>
                }
                <div className="flex gap-1">
                  ...
                  <Button size="sm" variant="outline" className="text-red-400 border-red-500/20 hover:bg-red-500/10" onClick={() => remove(t.id)}>
        ...
        <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div>
                <label className="block text-xs text-white/50 mb-1">Body BG</label>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-1">
                {FIELDS.map(f => (
                  <label key={f.key} className="flex items-center gap-2 text-sm text-white/60 cursor-pointer">
