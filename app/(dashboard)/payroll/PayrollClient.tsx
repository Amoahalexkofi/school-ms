    ...
    <main className="flex-1 p-6 space-y-5 bg-[#0f1015]">
      ...
      <div className="bg-[#111318] rounded-xl border border-white/[0.06] shadow-sm p-4">
        <div className="flex flex-wrap items-end gap-4">
          <div>
            <label className="block text-xs font-medium text-white/50 mb-1">Month</label>
            <select className="h-9 rounded-lg border border-white/[0.08] px-3 text-sm bg-[#111318] focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          {loaded && noPayslipCount > 0 && (
            <Button variant="outline" onClick={generateAll} disabled={bulkLoading} className="border-green-300 text-emerald-400 hover:bg-emerald-500/10">
              {bulkLoading
      ...
      {loaded && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { label: `Total Net Salary (${MONTHS.find(m => m.value === month)?.label} ${year})`, value: `₵${totalNetSalary.toLocaleString()}`, cls: "text-white/80" },
              { label: "Paid",    value: paidCount,    cls: "text-emerald-400" },
              ...
              <thead className="bg-[#0f1015] border-b">
                <tr>
                  {["Employee", "Department", "Designation", "Basic (₵)", "Allowances (₵)", "Deductions (₵)", "Net (₵)", "Status", ""].map(h => (
                    <th key={h} className="text-left px-4 py-3 font-medium text-white/50 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y">
                {rows.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-white/30">
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-[#0f1015]">
                      ...
                      <td className="px-4 py-3 text-emerald-400">{p ? Number(p.totalAllowance).toLocaleString() : "—"}</td>
                      <td className="px-4 py-3 text-red-400">{p ? Number(p.totalDeduction).toLocaleString() : "—"}</td>
                      ...
                      <td className="px-4 py-3">
                        {p ? (
                          ...
                          <span className="text-xs text-white/30">No payslip</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        {p ? (
