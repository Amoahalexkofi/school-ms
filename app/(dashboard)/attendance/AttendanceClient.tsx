...
const KV_STYLE: Record<string, string> = {
  ...
  H:  "bg-blue-500/10   text-blue-400   border-blue-300",
...
export function AttendanceClient({ sessions, classSections, attendanceTypes }: Props) {
  ...
  return (
    <main className="flex-1 p-6 space-y-5 bg-[#0f1015] min-h-0">
            ...
            <div className="bg-[#111318] rounded-xl border border-white/[0.06] shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-[#0f1015] border-b">
                  <tr>
                    <th className="text-left px-4 py-3 font-medium text-white/50 w-10">#</th>
                ...
                <tbody className="divide-y">
                  {enrollments.map((enr, idx) => {
                    ...
                    return (
                      <tr key={enr.id} className={`hover:bg-[#0f1015] ${selectedType?.keyValue === "A" ? "bg-red-500/10/30" : ""}`}>
                        <td className="px-4 py-3 text-white/30 text-xs">{idx + 1}</td>
                        <td className="px-4 py-3 text-white/40 text-xs font-mono">{enr.rollNo ?? "—"}</td>
                        <td className="px-4 py-3">
                          <span className="font-medium text-white/80">
                            {s.firstName} {s.middleName ? s.middleName + " " : ""}{s.lastName}
                          </span>
                          {s.gender && <span className="text-xs text-white/30 ml-2">{s.gender}</span>}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            className="w-36 h-7 rounded border border-white/[0.06] px-2 text-xs text-white/60 focus:outline-none focus:ring-1 focus:ring-blue-400"
                          />
                        </td>
                      </tr>
                    );
