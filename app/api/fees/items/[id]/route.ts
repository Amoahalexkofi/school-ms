...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await (prisma as any).feeGroupItem.update({ where: { id }, data: body });
  return NextResponse.json(item);
}

export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const count = await (prisma as any).feeDeposit.count({ where: { feeGroupItemId: id } });
  if (count > 0) return NextResponse.json({ error: "Has payment records — cannot delete" }, { status: 409 });
  await (prisma as any).feeGroupItem.delete({ where: { id } });
