...
export async function POST(req: NextRequest) {
  try {
    ...
    const i = await (prisma as any).item.create({ data: { name: body.name.trim(), categoryId: body.categoryId || null, supplierId: body.supplierId || null, storeId: body.storeId || null, quantity: parseInt(body.quantity) || 0, lowStockAlert: parseInt(body.lowStockAlert) || 5, unit: body.unit || null } });
