...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === "start") {
    ...
    const existing = await ((await getDb()) as any).examAttempt.findUnique({
      where: { onlineExamId_studentId: { onlineExamId, studentId: student.id } },
    });
    if (existing) return NextResponse.json(existing);

    const attempt = await ((await getDb()) as any).examAttempt.create({
  ...
  if (action === "submit") {
    const attempt = await ((await getDb()) as any).examAttempt.findUnique({
      where: { onlineExamId_studentId: { onlineExamId, studentId: student.id } },
    });
    ...
    const examQuestions = await ((await getDb()) as any).onlineExamQuestion.findMany({
      where: { onlineExamId },
    ...
    for (const eq of examQuestions) {
      ...
      await ((await getDb()) as any).examAnswer.upsert({
    ...
    const updated = await ((await getDb()) as any).examAttempt.update({
      where: { id: attempt.id },
