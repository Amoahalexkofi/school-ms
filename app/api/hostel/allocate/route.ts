...
export async function POST(req: NextRequest) {
  try {
    ...
    const room = await (prisma as any).hostelRoom.findUnique({ where: { id: roomId }, include: { _count: { select: { allocations: true } } } });
    if (room && room._count.allocations >= room.capacity)
      return NextResponse.json({ error: "Room is at full capacity" }, { status: 409 });
    const a = await (prisma as any).hostelAllocation.upsert({
      where: { studentId },
